...
//    final int SENDQUAT = 3;
    //accelerometer related
    protected boolean wantsAccelerometer = false;
    protected boolean wantsCompass = true;
//    private static SensorManager mSensorManager; 
//	private static Sensor mRotationVector;
//    protected int mSensorDelay = SensorManager.SENSOR_DELAY_NORMAL; //other options: SENSOR_DELAY_GAME, SensorManager.SENSOR_DELAY_FASTEST, SensorManager.SENSOR_DELAY_NORMAL and SensorManager.SENSOR_DELAY_UI
     ...
     * Now I use this for ORIENTATION_VECTOR 
     ...
     * @return True if the event was handled.
     */
    public boolean sensorEvent(int sensorType, float values0, float values1, float values2, float values3)
    {
    	return true;
    }
   ...
   //     multiData = new float[MULTI_DATA_STRIDE*MULTI_MAX_INPUTS]; 
    ...
    protected void onResume()
    {
        ...
        //ugly init statement
        //moved up here so it might be initialized togehter?
        //	smoothedQuaternion.newAverage();
		}
    */
...
/*	public void onSensorChanged(SensorEvent event) 
	...
	//	if (event.accuracy == SensorManager.SENSOR_STATUS_UNRELIABLE) 
		...
		if (event.sensor.getType() == Sensor.TYPE_ROTATION_VECTOR)
		{
			if (firstScan)
			{
				SensorManager.getQuaternionFromVector(foo, event.values);
				//SmoothQuaternion smoothedQuaternion = (SmoothQuaternion) foo;
				smoothedQuaternion.setQuat(foo);
				firstScan = false;
			}
			else
			{
